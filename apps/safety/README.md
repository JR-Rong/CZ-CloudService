...
示例地址 `203.0.113.104/30` 属于文档网段，脚本会拒绝把它应用到真实设备。到现场后必须换成 IDC 分配的真实完整地址；该 `/30` 的固定关系是 `.104` 网络地址、`.105` IDC 网关、`.106` 本方设备、`.107` 广播地址。
...
若现场没有可靠互联网，应提前在一台全新的、同架构 Ubuntu 24.04 机器上运行 `prepare-bundle --output-dir offline-debs`，把生成的 `.deb` 与 `SHA256SUMS` 一起带到现场，并把 `PACKAGE_MODE` 设为 `offline`。不要在已安装这些包的日常主机上生成依赖包，否则 apt 可能不会重新下载所有依赖。
